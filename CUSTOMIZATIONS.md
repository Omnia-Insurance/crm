...
| `packages/twenty-front/src/modules/auth/states/tokenPairState.ts`                                                                  | Auth cookie domain derived from current hostname's parent (strips leftmost label); lets `omniaagent.com` dashboard read the cookie set by `crm.omniaagent.com` for cross-app SSO |
...
| `packages/twenty-server/src/engine/core-modules/auth/types/social-sso-state.type.ts`                                               | Added optional `postSignInRedirect` to OAuth state so the redirect target survives the Google/Microsoft OAuth round-trip |
| `packages/twenty-server/src/engine/core-modules/auth/strategies/google.auth.strategy.ts`                                           | Propagates `postSignInRedirect` into and out of OAuth state for Google |
| `packages/twenty-server/src/engine/core-modules/auth/strategies/microsoft.auth.strategy.ts`                                        | Propagates `postSignInRedirect` into and out of OAuth state for Microsoft |
| `packages/twenty-server/src/engine/core-modules/auth/services/auth.service.ts`                                                     | Trust-checks `postSignInRedirect` against `FRONTEND_URL` and only forwards safe values into the post-auth `tokenPair` URL |
| `packages/twenty-front/src/modules/auth/sign-in-up/components/internal/SignInUpGlobalScopeFormEffect.tsx`                          | After cookie persistence, redirects to a trusted `postSignInRedirect` (defense-in-depth trust check) so sibling apps can bounce users home after sign-in |
| `packages/twenty-front/src/modules/auth/components/VerifyLoginTokenEffect.tsx`                                                     | After single-workspace `/verify` cookie persistence, redirects to a trusted `postSignInRedirect` — gated on reactive token-pair state so failed verifications never trigger an external redirect |
| `packages/twenty-front/src/modules/auth/hooks/useAuth.ts`                                                                          | `buildRedirectUrl` (and `handleGoogleLogin` / `handleMicrosoftLogin`) accept + forward `postSignInRedirect` onto `/auth/google` and `/auth/microsoft`, where the strategies read it into OAuth state |
| `packages/twenty-front/src/modules/auth/sign-in-up/hooks/useSignInWithGoogle.ts`                                                   | Reads `postSignInRedirect` from URL and forwards into the Google OAuth kickoff URL |
| `packages/twenty-front/src/modules/auth/sign-in-up/hooks/useSignInWithMicrosoft.ts`                                                | Reads `postSignInRedirect` from URL and forwards into the Microsoft OAuth kickoff URL |
| `packages/twenty-front/src/modules/auth/sign-in-up/components/internal/SignInUpExternalRedirectEffect.tsx`                         | **NEW** — when an already-authed user lands on `/welcome` with a trusted `postSignInRedirect`, bounce them back instead of dropping on CRM home. Covers the already-authed path that `SignInUpGlobalScopeFormEffect` / `VerifyLoginTokenEffect` don't (those only fire on fresh-token flows) |
