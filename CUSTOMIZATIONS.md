...
| `packages/twenty-server/src/engine/api/graphql/graphql-config/hooks/use-cached-metadata.ts`                                                                                               | Core-view metadata cache keys must stay user-scoped for both `FindAllCoreViews` and `FindFieldsWidgetCoreViews`               | Core view visibility is user-dependent; caching it per workspace only would leak the wrong results                                                             |
