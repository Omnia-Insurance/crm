...
| `packages/twenty-front/src/modules/object-record/record-field/ui/meta-types/input/hooks/useAddNewRecordAndOpenSidePanel.ts`                                                               | Creates draft in store with FK prefills instead of immediately creating related record; prefills Agent from current workspace member's agent profile       | Routes relation "+" button through draft creation; mirrors `useCreateNewIndexRecord` so member-role agents see Agent prefilled when creating from a relation  |
...
| `packages/twenty-front/src/modules/command-menu-item/server-items/common/hooks/useCommandMenuItemsFromBackend.tsx`                 | Applies object-aware create/go-to labels, blue CTA styling, deactivated object filtering, and LAYOUTS permission gate on layout editing |
